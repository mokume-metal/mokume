...
- 想定だけの API を先回りで作らない。機能は実際の作品制作で踏まれた必要から正当化する (ADR-0001 原則 4)。**その作品は別リポジトリで作り、フェーズの入口で同期する** ([ADR-0022](docs/decisions/0022-production-track.md))。認めるのは「作品制作で書けなかった / 書けたが歪んだ」と「既にある規範が要求する一貫性の欠け」の 2 つで、**`Sketches/` で踏んだものは実需に数えない** (面から逆算して書かれるので循環する)。実需由来かどうかは `Feature` 型の Issue 本文が持つ — どの作品で何を作ろうとして何ができなかったかを書き、作品リポジトリへリンクする (ラベルは足さない)
