...
絵を貼って塗る (`texture`) か断片 (`shader`) を効かせた `circle` / `ellipse` / `arc` の太い輪郭も、同じ理由で周の斜めの区間が太っていたので、あわせて直しました。

`vertex` で置いた点と、曲線の終点・通過点 (`curveVertex` に渡した点)、扇の中心と弧の両端は、これまでどおり角として `strokeJoin` に従います。
