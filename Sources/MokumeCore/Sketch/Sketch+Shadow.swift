...
extension Sketch {
    ...
    /// 影の内外を問わず効く。
    ///
    /// **焼き付けはフレームの終わりに 1 度だけ走り、そのときに置かれている光から落とす
    /// 光を選ぶ。** 立体の陰影は置いた時点の光で決まるが、影はそうではない。`draw()` の
    /// 途中で ``noLights()`` を呼び、向きを持つ光を置き直さずにフレームを終えると、
    /// **それより前に置いた立体の影も含めて、そのフレームの影が丸ごと出ない** (この
    /// とき初回だけ警告が出る)。手元の表示を立体の後に描くときは、光を取り除かずに描くか、
    /// 描き終えてから向きを持つ光を置き直す。
    ///
