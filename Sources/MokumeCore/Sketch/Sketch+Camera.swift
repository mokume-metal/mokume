...
extension Sketch {
    ...
    /// 描いた平面の図形とぴったり重なる。
    ///
    /// **戻すのは視点 (見る位置・見ている先・上方向) だけである。** ``ortho()`` や
    /// ``perspective(_:_:_:_:)`` で決めた写し方は、`camera()` を書いた後も効き続ける —
    /// どこから見るかと、どう写すかは別の指定なので、片方を戻したときにもう片方まで
    /// 既定へ戻ると驚きになる。写し方も既定の透視へ戻すときは ``perspective()`` を
    /// 合わせて書く。
    ///
    ...
    ///   見る位置と見ている先が同じ・上方向が視線と重なるといった成り立たない視点や、
    ///   ``perspective(_:_:_:_:)`` / ``ortho(_:_:_:_:_:_:)`` が受けない投影 (写す範囲が
    ///   潰れている・画角が 0 以下か π 以上・横 ÷ 縦の比か手前の面までの距離が 0 以下・
    ///   数でない値) を持つ値は、警告して丸ごと捨て、それまでの視点のまま続ける。
    ///
