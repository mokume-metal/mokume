...
extension Sketch {
    ...
    /// ## 同じ絵は一度しか読まない
    ///
    /// 二度目からは探すのも復号するのも省く ([#886])。**それでも `setup()` で読むこと** —
    /// 返る絵は毎回新しいので、面を作って送る分は毎回かかる。
    ///
    /// 毎回新しいということは、**読んだ絵を塗り替えても次の読み込みには影響しない**
    ...
    /// たとえば 64 画素四方の絵の `(32, 0, 64, 64)` を `128×128` へ置くと、置き先の右半分
    /// (絵の右端を越えた所) には何も置かれず、重なった分 (絵の右半分) が 2 倍の
    /// `64×128` で置き先の左半分に出る。
    ///
    ...
    /// }
    ///
    /// 3 つの数で置く ``image(_:_:_:)-(Image,_,_)`` (描き場所を置く ``image(_:_:_:)-(Canvas,_,_)``
    /// も) にも読み方は効く。ただし大きさは変わらず、決まるのは (`x`, `y`) が絵の左上の角か
    /// 中心かだけである。
    ///
