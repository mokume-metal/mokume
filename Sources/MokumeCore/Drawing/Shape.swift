...
/// 3D を足すときに広がるのは頂点の中身であって、形の種類ごとの対応表ではない。
/// 「2D では効くが 3D では黙って無視される」機能が生まれる場所が無い。
public struct Shape {
    ...
    init(vertices: [ShapeVertex], runs: [Run]) {
    ...
    public static func group(_ shapes: [Shape]) -> Shape {
        ...
        return Shape(vertices: vertices, runs: runs)
    }
    ...
    private static func append(_ run: Run, to runs: inout [Run]) {
        if var last = runs.last, last.mode == run.mode, last.texture === run.texture,
            last.textureKind == run.textureKind, last.start + last.count == run.start
        {
