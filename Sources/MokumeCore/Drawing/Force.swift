...
/// ## 力は加速度として足し合わさる
///
/// どの力も**加速度** (1 秒あたりの速さの変化) を返し、効かせた力の分を足し合わせたものが
/// その粒の加速度 a になる。粒は 1 フレームごとに、**先に速度を、進めた速度で位置を**進める
/// (半陰的オイラー)。刻み Δt は ``Sketch/deltaTime``:
///
/// ```
/// v ← v + a·Δt
/// x ← x + v·Δt
...
public enum Force: Equatable, Sendable {
    ...
    ///
    /// 加速度は −`amount` × 速度で、1 フレームで速度は (1 − `amount`·Δt) 倍になる。
