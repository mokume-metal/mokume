...
#### 改訂 (2026-09-07) — 在庫は着手を待たずに作ってよい

**当初の決定**は「**在庫は着手のたびに 1 件ずつ固める**」だった。理由は一括トリアージが「中身を見ずに印を付ける」ことになり、追補の基準を正面から破るためである。**その理由は動いていない** — 変わったのは、在庫を作る仕事そのものが着手とは別に立つようになったことである ([ADR-0036](0036-unattended-issue-processing.md) 決定 3)。

禁じ続けるのは**中身を見ずに印を付けること**だけで、「着手の直前であること」ではない。1 件ずつ調べて完了条件を本文へ書いた結果に印を付けるなら、それが着手の 1 時間前でも前夜でも同じことである。

**着手時の再チェックが後ろで受ける。** 先取りした在庫は着手までの間に陳腐化しうるが、[ADR-0031](0031-triage-as-the-single-gate.md) 決定 4 が着手の瞬間に完了条件を現行のコードと突き合わせる。陳腐化はここで拾う。

**踏んで起票した障害は、起票の瞬間に行き先を決める** ([ADR-0036](0036-unattended-issue-processing.md) 決定 6)。上の基準の「自分が起票した Issue であること」を後から判定できるセッションは無い — [ADR-0003](0003-agent-identity-separation.md) が identity を分離したのは PR の作成だけなので、エージェントが起票した Issue も author はメンテナになる。**この経路が開いているのは起票したその瞬間だけ**である。
