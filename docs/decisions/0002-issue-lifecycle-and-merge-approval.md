...
採用 (2026-08-26) / 一部置換 (→ [ADR-0003](0003-agent-identity-separation.md)): 決定 4 / 一部置換 (→ [ADR-0004](0004-issue-classification-by-issue-type.md)): 決定 1 の分類の表現 / 一部置換 (→ [ADR-0031](0031-triage-as-the-single-gate.md)): 決定 1 のラベル分類・決定 3 のルーティング・決定 5 / 改訂 (2026-08-31): 決定 1 にエージェント自身の起票を含める
...
#### 追補 (2026-08-31) — エージェント自身の起票も同じ基準で扱う

上の表はスクリプトによる定型の自動起票だけを挙げていたが、**基準が分けているのは「人か機械か」ではなく「完了条件を知っているか」である** ([#621](https://github.com/mokume-metal/mokume/issues/621))。エージェントが作業中に踏んで起票した障害は、起票の時点で完了条件を知っていることが多い。知っているのに付けられないのは、基準の当てはめ漏れであって基準の帰結ではない。

エージェントが自分で `verify:` を付けてよいのは、**次を両方満たすときだけ**である:

- **自分が起票した Issue であること** — 人間や他のエージェントが書いた Issue の完了条件を、後から来た者は知らない
- **本文に完了条件が書けていること** — 「どうなれば解消か」を具体的に書けなかったなら、それは知らないということである
...
`scripts/triage.sh` が `verify:` を付けないのも同じ基準の帰結である — 人間が書いた任意の Issue の完了条件を、triage は知らない。知らない者が付ければ、印だけが先に立って合意が後から追いかけることになる。**上の追補はここを広げない** — エージェントが付けてよいのは自分が起票した Issue だけで、triage が扱うのは他人が書いた Issue である。
