...
| 区画へ JSON を置く口 ([#989](https://github.com/mokume-metal/mokume/issues/989)) | 6 箇所が銘々 `JSONEncoder` を組み、失敗を `try?` で捨てる / `warn` で言う / 投げる の 3 通りに分かれていた。**畳んだのは「置けなくなったことを 1 度だけ言い、回復したら数え直す」の対**である — 後者を落とすと**以後 1 度も言わなくなり** ([#221](https://github.com/mokume-metal/mokume/issues/221) が塞いだ穴に戻る)、落としてもコンパイルは通る。**文面は主語だけを差し込む** — 動詞や語幹を組むと組んだ文のほうが壊れる ([#947](https://github.com/mokume-metal/mokume/issues/947)) |
...
| 原子的書き込みの 2 実装 ([#989](https://github.com/mokume-metal/mokume/issues/989)) | `MokumeCore/Exchange/AtomicFile` と `MokumeCLI/AtomicWrite` は逐語同文だが、**互いに一致している必要が無い** — それぞれが独立に「読み手が書きかけを掴まない」([ADR-0018](0018-observation-and-control-surface.md) 決定 3) を満たせば足り、片方が崩れてももう片方は影響を受けない。線の両側を担う綴りの写しとは違う。畳むには `MokumeCore` から `public` で出すしかなく (道具は umbrella にしか依存しない)、**スケッチの作者の補完候補が 1 つ増える** — 作者が触るものではないので[ADR-0020](0020-api-naming-and-surface.md) の面の規範に照らして損である |
