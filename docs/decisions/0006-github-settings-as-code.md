...
#### 追補 (2026-08-31) — 必須チェックを消すときだけ、適用が先

「定義ファイルの PR → merge → `--apply`」という順序は、**操作の向きによって成り立ったり成り立たなかったりする** ([#622](https://github.com/mokume-metal/mokume/issues/622))。

| 操作 | 適用が後でよいか |
| --- | --- |
| 足す | よい。適用までは新しい要求が効かないだけで、**安全側に倒れる** |
| 変える | よい。古い要求が効き続けるだけ |
| **消す** | **だめ。消す PR 自身が、消そうとしている必須チェックを満たせない** |

[#620](https://github.com/mokume-metal/mokume/pull/620) で実際に踏んだ。`human-approval` を必須チェックから外しつつ、それを報告する `approval-signal` ジョブを同じ PR で削除したので、GitHub 側が要求し続けるのに報告する者が居ない状態になり、承認が付いても `Expected — Waiting for status to be reported` から動かなくなった。

**消すときは merge の前に `--apply` を打つ。** 鮮度検査は作業ブランチを `editing` (手元の定義が main にまだ入っていない) と判定するので止まらない — 止まるのは `stale` のときだけである ([#425](https://github.com/mokume-metal/mokume/issues/425))。適用から merge までの短い間、日次のドリフト検査は main の定義とのずれを見て赤くなりうるが、これは [#381](https://github.com/mokume-metal/mokume/issues/381) が言う「定義だけが先行するときの赤は催促」の**逆向き**で、その PR が入れば解消する。

**`local-render` は `human-approval` より詰みやすい。** 承認待ちを表す status なら「手で success を打って抜ける」という発想が一応できる (merge queue が別の sha で再検証するので実際には効かないが)。`local-render` は**手元で実際に絵を回した記録**なので、手で打つのは記録の偽造にあたる ([ADR-0019](0019-drawing-verification.md) 決定 1 が守っている不変条件そのものを壊す)。消す・改名するときは、`--apply` が先という手順が唯一の道になる。

**機構は足さない。** `bash scripts/apply-rulesets.sh` (`--apply` なし) を打てば、必須チェックが減っていることは差分にそのまま出る。#620 で詰んだのは道具が無かったからではなく、打つべき瞬間を知らなかったからである — 知識の欠落に機構で応えるのは [ADR-0008](0008-mechanism-needs-demonstrated-harm.md) が戒めている形にあたる。同じ詰みをもう一度踏んだら、そのときは実害が 2 件になるので `check-rulesets.sh --shape` の責務を広げる案へ進む。
