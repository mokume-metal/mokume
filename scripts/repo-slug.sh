...
#                                        (定義ファイル・雛形) を指すときはこれ
#
# **`$0` は使わない。** source されると呼び出し側を指し、cwd にも依存する。以前 5 本が
# `cd "$(dirname "$0")/.."` を使っていた (#820 で寄せた)。`pwd -P` を付けるのは
# symlink 経由で起動されたときに実体の場所へ解くためである。
#
# 下の repo_of_dir は**前者**の問いに答える (与えられたディレクトリの origin を見る)。
