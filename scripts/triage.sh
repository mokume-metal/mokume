...
#
# **エージェントが自分の起票に verify: を付けてよくなった後も、ここは付けない** (#621)。
# 決定 1 の追補が許したのは「自分が起票し、本文に完了条件を書けた Issue」までで、
# triage が扱うのは**他人が書いた任意の Issue** である。その完了条件を triage は
# 知らないので、基準はそのまま「付けない」に落ちる。
#
