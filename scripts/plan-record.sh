...
# --- 検査 -------------------------------------------------------------------
# 秘密情報の探索は scripts/secret-scan.sh が持つ (#819)。**プランに固有ではない** —
# 問うているのは「この本文を GitHub へ出してよいか」で、プランかどうかとは関係がない。
# BLOCK は投稿用ファイルを作らせず、WARN は判断をエージェントへ委ねる
# shellcheck source=scripts/secret-scan.sh
...
# 「plan-record.sh が知りたいのはこれ」と名指ししている (#820)。
#
