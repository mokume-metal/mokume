...
/// 粒を大量に飛ばし、力で曲げる。
///
/// **見どころは、円盤が「円盤を描く命令」なしに出てくること。** 置いているのは
/// 輪の上へ粒をばらまく命令だけで、出る向きは毎回でたらめである。円盤の形も回る速さも、
/// 4 つの力の足し算から出ている — 引く力が内へ、渦が周りへ、抵抗が速さの上限を決め、
/// 弱い重力が上下の対称を崩す。
///
...
final class SparksAndForces: Sketch {
    ...
    /// 粒を生む輪の半径。**釣り合う半径の近くに置く**ので、生まれた粒はすぐ円盤に乗る。
    ///
