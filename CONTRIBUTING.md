...
2. fork して枝を切り、`make ci-check` を通してから PR を出す。本文はテンプレートを埋める
3. 残りはメンテナが引き取る:
   - ラベル (`verify: triaged` / `no-issue` / `no-visual-change`) を付け、検査を再評価させる
   - 描画に触れる PR の `local-render` を、メンテナの手元で回して報告する
   - doc コメントの例を直した場合の撮り直し (`make example-shots`)
   - merge queue へ入れる

赤い検査が残っていても、上の 4 つに当たるものなら直さなくてよい。
...
- 個別の設計判断: [docs/decisions/](docs/decisions/) の ADR (書き方は同じディレクトリの AGENTS.md)
