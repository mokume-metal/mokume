...
// 触る前に待っていないということである。
//
// **読むのは原文なので、doc やコメントでその綴りに触れただけでも数える。** 触らない側の
// ファイルが「触るのは呼ぶ側の仕事である」と説明したくなったときは、綴りそのものを書かずに
// 言い換える (`Output/Brightness.swift` がそうしている)。狭めない — 綴りを避けるのは数秒だが、
// コメントを外して読む形にすると「コメントに見えるコード」を取りこぼす側の危険が出る。
