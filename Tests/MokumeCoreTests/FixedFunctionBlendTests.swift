...
struct FixedFunctionBlendTests {
    ...
    func eachBlendKindHasItsOwnPipeline() throws {
        ...
        let groups =
            ...
            + ShapePipeline.formFlagCombinations.map { pipeline.formStates(for: $0) }
    ...
    ///
    /// 塗りの中心は画素の角 (32, 32) に乗り、半径 10 の円の渡しは中心から 9.5〜10.5 画素に
    /// ある。画素 i の中心は i + 0.5 なので、画素 43 と 20 の中心は円の中心から 11.5 画素
    /// 離れる。クアッドは余白 2 画素ぶん (12 画素まで) を覆うので、**そこは断片が走るのに
    /// 何も塗らない**。そこを指す — クアッドの外を見ても断片が 1 度も走らないので、余白の
    /// 扱いは試せない。
    private static let marginSpots = [(43, 32), (20, 32), (32, 43), (40, 40)]
