...
struct ShapeFormulaTests {
    ...
    /// 結ばれる」と約束している。
    ///
    /// **刻みは 2 にする。** 各辺の継ぎ目は t = 0.5 の 1 点だけになり、張り具合 1 ならそれが
    /// 辺の中点に乗る。張り具合が効いていなければ (0 のまま)、最初の辺を決める 4 点は
    /// (A, A, B, C) なので、継ぎ目は Catmull-Rom の t = 0.5 の (8A + 9B − C) / 16 = (24, 10) へ
    /// 2 画素ふくらむ (A〜D は下の `corners`)。ほかの刻みでは見られない:
    ///
    /// - 刻み 1 は t = 1 の点 (次の通過点) しか置かないので、張り具合によらず折れ線になる
    /// - 既定の刻み 20 では、始点から 0.23 画素の所に継ぎ目が来て、そこを埋める正方形が
    ///   丸い端点の外へ出る (2 画素違う)。張り具合ではなく折れ目の埋め方の話で、
    ///   [#1409] が扱う
    ///
    /// **3 本の辺を軸に沿わせ、太さを奇数 (3) にする。** 継ぎ目を埋める正方形が帯の中に
    /// 収まり、帯の縁は画素の境目に乗る — どの画素の中心も縁の上に来ないので、経路の違う
    /// 2 つの絵が縁の判定で割れない。
    ///
