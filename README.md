...
**環境の前提** (`What the environment provides` — OS・機種・描く道具・道具立て)・
**手元の状態** (`What is here` — スケッチ・組み上げた跡とその在処・共有のビルド置き場・
最後の作り直し)・**起動の瞬間に決まるもの** が同じ出力に並ぶ。前提と状態を並べないと、「そもそも前提を満たしていない」と「前提は満たしているが
区画が割れている」を分けられない。

**何も直さない。** 区画を作ったり走っているものを止めたりはせず、状態と読み方だけを出す。
読めなかったものは `cannot tell` と名乗り、こちらで断定しない。
